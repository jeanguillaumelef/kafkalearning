        ...
        public static void Run_Consume(string brokerList, List<string> topics, CancellationToken cancellationToken)
        {
                ...
                .Build())
            {
                ...
                try
                {
                    ...
                    //Add every topicPartition to the list
                    //var topicPartitionTimestamps = new List<TopicPartitionTimestamp>() { topicPartitionTimestamp };
                    //var earliestOffsetCompareToDate = consumer.OffsetsForTimes(topicPartitionTimestamps, TimeSpan.FromMinutes(1));
                    ...
                    while (true)
                    {
                        try
                        {
                            ...
                            //Static offset given
                            if (consumeResult.Offset < offset.Value)
                            {
                                consumer.Assign(topicPartitionOffset);
                            }

                            //Dynamic offset (latest)
                            //if (earliestOffsetCompareToDate.First(x => x.Partition == consumeResult.Partition).Offset.Value >= consumeResult.Offset)
                            //{
